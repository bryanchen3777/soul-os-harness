...
### MS-2（Multimodal Perception 实作：STT + Camera 工具层接入）【CANDIDATE，待主大脑指派】

| 条目 | 状态 | 要点 | 相关 commit |
|------|------|------|-------------|
| **MS-2（实作）** | ⏳ CANDIDATE（未指派） | MS-1 设计锁定后遗留的实作项：工具层三处 additive（observe 关键词 / explicit group map / 能力定义）+ `VALID_SOURCES` additive 扩展 `audio_input`/`camera_capture` + 自研薄 MCP 封装 `audio-stream-mcp` / `camera-mcp` + faster-whisper small 本地离线 ASR 接入。**DoD（Definition of Done）**：语音/相机事件经 MCP 工具 → `observe_environment` 组 → Volition Gate 审核后回流感知，**不直通 USER_MESSAGE**。 | —（待 MS-2 工单） |
...
- Current HEAD: `172bca0` (docs: multimodal perception contract (MS-1))
- MS-1 commit: `172bca0` (docs: multimodal perception contract (MS-1); **Current HEAD**)
