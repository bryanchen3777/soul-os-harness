...
- Current HEAD: `001218f` (docs(eh-4): add native tools immunity and spoken ladder calibration (EH-4-AMEND-2)) — **EH-4-AMEND-2 認知心智架構修訂 2（原生免疫閘與口語校準，DESIGN ONLY）**（1 檔：`docs/EH-4-EPISTEMIC-MIND-CONTRACT.md` +224/-48）。**核心修訂（封堵實戰死角）**：① **§2.2 原生常識天然免疫閘（FSA-0，NT-1~NT-6）**：`P0 ∈ (pack.native_tools ∪ pack.native_basis) ⟹ FSA (None)`；判定池為實例包四軸聯集派生欄位，完全匹配立即終止差量對撞，0 黑名單，100% 杜絕鐵鍋／風箱等原生器具被當異界物對撞。② **§2.2 口語無介詞解析階梯與 v1 保守邊界（SL-1~SL-5 / UR-1~UR-4）**：階梯 1（P0 介詞錨定）→ 階梯 2（P1-Spoken 評價主語/動作謂語錨定）→ 階梯 3（保守放棄回 `None`，**Known Under-Recall by Design**，寧可漏召口語也嚴禁盲目引入模糊 NLP 依存語法器）。③ **§1.2 & §2.4 本體隱喻多錨點分流矩陣（AC-1~AC-6）**：規格化四類次級物理特徵封閉集合（`enclosed_heat` 封閉石爐、`forced_air_heat` 風箱熱道、`open_flat_heat` 鐵板、`cold_preservation` 地窖），使微波爐／氣炸鍋／平底電熱鍋／冰箱在原生心智映射至不同生活器具，破除單一烘爐模板。④ **§2.3 當輪女僕關切度量約束（Anti-Nagging Guard，IT-7~IT-10）**：在場親自操作或即將食用 ∧ (實質危害懷疑 ∨ 能替換繁重家務) 雙條件滿足才允許當輪發問；背景提及（如「辦公室有微波爐」）嚴禁發問；探詢嚴格上限一句、安靜得體。**驗收**：單一文檔修訂，0 程式碼、0 測試、0 配置、0 persona 改動；0 踩 Frozen Contracts。**Current HEAD**）
