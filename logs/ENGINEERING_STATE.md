...
**Next work**: P1-C2（Integration / Boundary Gate）。P1-C2 必答（reviewer 記帳）：staging→ingest 執行流 wiring（agent content transport）+ artifact claim 空 refs 非空下限 + evidence claim→verify（P1-B D8 承接）+ 路由切換勿誤用 mock 面 execute_work + headless approval policy（fail-fast deny 而非 ask，防 hang）。
