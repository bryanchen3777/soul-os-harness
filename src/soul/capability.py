...
定位（照 docs/SOUL-CAPABILITY-AWARENESS-DESIGN.md，CA-1 已鎖）：
  - **Definition**（系統事實，machine-readable，唯一權威源）：``CAPABILITY_DEFINITIONS``
    常數。v1 只有 ``communicate``（→ proactive_message）。不上 YAML。
...
v1 投影規則（死規則，工單已鎖）：
  ...
  - deterministic：無 relevance score / decay / random / confidence 動力學；
    v1 全量投影（只有 1 個 capability），按 dict 插入順序。
...
class CapabilityDefinition:
    ...
    - ``id``：machine-readable，穩定、小寫、無空格。v1 只有 ``"communicate"``。
...
def project_capabilities(
    ...
    *,
    ...
    規則（v1 死規則）：
      ...
      2. 全量投影（v1 只有 1 個），按 dict 插入順序（deterministic）。
