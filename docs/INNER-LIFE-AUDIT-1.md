...
> **本區塊為 2026-09-13 追加的正式更正。原文全部保留（0 刪除、0 改寫既有段落與數字）。更正依據：`docs/ELEVATION-EVIDENCE-CONTRACT.md`（commit `3eee636`，READ-ONLY 重測 ＋ 生產日誌 635 筆 `[elevate] skip` 全數解析）。**

**① 被更正的前提（§3.5 與 §10 所述）**：「world 桶每次都能拿 **1,363 份獨立證據**」→ **此前提是錯的。**

- **1,363 實為 `node_elevated` 事件的 `pattern_node_ids` 長度＝pattern 數（同維度 pattern 總數），不是 `_count_independent_evidence()` 的回傳值。**

**② 以同一演算法在同一時點重算 = `1`。**

- 完整數字鏈：`belief` soul 節點 **658** → 指向 soul 的邊 **1,374** → 已消化鍵（`consumed_keys`）**1,356** → pattern 邊 **1,376** → **載入引擎 = 1** → **獨立證據數 N = 1**。
- 與生產日誌 `[elevate] skip agent=default candidate=belief: 1 < 2` **逐字相符**。

**③ 因此根因被更正。**

- **不是**「證據獨立性定義太嚴」——原 §3.5 的框架「在世界桶是『太鬆』、在 agent 端是『不可能』」與 §6 P1-2 的「agent 端永不可能升華／另設門檻」立論，均建立在上列錯誤前提之上。
- **而是**：**反重複消化過濾器（`consumed_keys`）＋ 每輪僅新增 1~2 份可見證據的產率，把可見證據池結構性鎖死在 ≤ 1，而門檻恰好是 2。**
- `file:line`：`src/inner_life/elevation_adapter.py:539-542` / `:555-556`（`consumed_keys`）；上游 `soul-elevation/engine.py:92` `DEFAULT_ELEVATE_MIN_EVIDENCE = 2`。
- **→ world 桶與 agent 桶是同一個卡點**，不是「一邊太鬆、一邊結構上不可能」。

**④ 更正依據的實測強度**（`docs/ELEVATION-EVIDENCE-CONTRACT.md`，`3eee636`）：READ-ONLY 重測 ＋ 現行世代（`2026-09-12T22:39:13` 起）**635 筆 `[elevate] skip` 全數解析** → **`N ∈ {0, 1}`、`N ≥ 2` 出現 0 次**。

**⑤ 本更正不影響本審計報告的其他結論。** 關係軸 **P0-A**（100/100 `relational_band = stranger`）、意志軸 **P0-C**（7/10 角色 0 自主目標候選）、**升華節點 100% stub**、**Submission Gate 實況**（0 次 fail-closed 靜默丟棄）等，均經重測仍成立。（依契約重測：agent 桶 stub 節點 **550/550 = 100%**，比率不變；節點數隨時間由原報告的 513 累積上升。）
...
> ⚠️ 本處「1,363 份」已於 2026-09-13 更正，見文首更正區塊
